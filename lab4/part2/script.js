...
const albumCollection = [
    ...
    ,
    {
        ...
        title: ' teen spirit',
        artist: 'Witchz',
        tracks:['smells like teen spirit']
    },
    {
        id:4251,
        title: 'House of Cards',
        artist: 'Jutes',
        tracks:['Red Velvet','House of Cards']
    }
...
function getAlbumWithTrack(track) {
    return albumCollection.find(album => album.tracks && album.tracks.some(t => t.toLowerCase() === track.toLowerCase()));
}
//uncomment following test code after implementing the function
...
function updateAlbum(id, prop, value) {
    ...
    if (value === '') {
        delete album[prop]; // Remove property if value is empty
    } else if (prop === 'tracks') {
        if (!album.tracks) {
            album.tracks = [];
        }
